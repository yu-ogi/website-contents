...
function main() {
    const scene = new g.Scene({
        game: g.game,
        assetPaths: ["/image/circle.png", "/image/circle_touch.png", "/image/arrow.png"]
    });
    scene.onLoad.add(() => {
        // 壁を生成
        createWall(scene);
...
function createCircle(scene, parameter) {
    ...
    const circle = new g.Sprite({
        scene: scene,
        src: scene.asset.getImage("/image/circle.png"),
        srcWidth: 100,
        srcHeight: 100,
        width: parameter.appear.width,
        height: parameter.appear.height
    ...
    const circleTouch = new g.Sprite({
        scene: scene,
        src: scene.asset.getImage("/image/circle_touch.png"),
        srcWidth: 100,
        srcHeight: 100,
        width: parameter.appear.width,
        height: parameter.appear.height,
...
function createHajiki(scene, position) {
    ...
    hajiki.entity.onPointDown.add((event) => {
        ...
        arrow = new g.Sprite({
            scene: scene,
            src: scene.asset.getImage("/image/arrow.png"),
            srcWidth: 100,
            srcHeight: 100,
