...
function createTitleUI(scene) {
    ...
    const title = new g.Sprite({
        scene: scene,
        src: scene.asset.getImage("/image/title.png"),
        x: 128, y: titlePosY
    });
    scene.onUpdate.add(() => {
...
function createResultUI(scene) {
    ...
    const backBtn = new g.Sprite({
        scene: scene,
        src: scene.asset.getImage("/image/button_back.png"),
        x: 192, y: 272,
        touchable: true
    });
    ...
    const gameover = new g.Sprite({
        scene: scene,
        src: scene.asset.getImage("/image/gameover.png"),
        x: 80 + amp, y: 160,
        scaleX: 1 + scaleAdd,
        scaleY: 1 + scaleAdd
    });
...
module.exports = function() {
    ...
    scene.onLoad.add(() => {
        ...
        gameCore = new GameCore(
            scene.asset.getJSONContent("/text/map_data.json"),
            {
                x: game.width / 2,
                y: game.height / 2,
        ...
        scene.append(new g.Sprite({
            scene: scene,
            src: scene.asset.getImage("/image/background.png")
        }));

        const tile = new Tile({
            scene: scene,
            src: scene.asset.getImage("/image/map.png"),
        ...
        const bmpFont = new g.BitmapFont({
            src: scene.asset.getImage("/image/font16.png"),
            map: scene.asset.getJSONContent("/text/glyph_area.json"),
        ...
        const versionText = "ver " + scene.asset.getText("/text/version.txt").data.replace(/[\r\n]/g, "");
